    ...
    for frame in range(frame_number):
        for joint_idx, parent_idx in enumerate(joint_parents):
            if parent_idx == -1: # no parent joint
                ...
                parent_orientation = R.from_quat(global_joint_orientations[frame, parent_idx])
                offset_vector = joint_offsets[joint_idx][0]
                rotated_offset = parent_orientation.apply(offset_vector)
                global_joint_positions[frame, joint_idx] = global_joint_positions[frame, parent_idx] + rotated_offset
