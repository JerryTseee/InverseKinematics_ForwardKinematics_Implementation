    ...
    if method == 'ccd':
        ...
        for _ in range(iteration_num):
            for current_idx in range(end_idx - 1, 0, -1):
                ...
                # calculate the angle between two vectors (using dot product to get the angle!)
                dot_product = np.vdot(vec_current_to_end, vec_current_to_target)
                dot_product = np.clip(dot_product, -1.0, 1.0) # ensure between -1 and 1
                ...
                chain_orientations[current_idx] = rotation_vec * chain_orientations[current_idx]
